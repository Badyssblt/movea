...
# Chemin du script (pour supprimer les archives)
SCRIPT_DIR="$(pwd)"

# ----------------- FONCTION DEPLOIEMENT FRONTEND -----------------
deploy_frontend() {
    echo "🔨 Compilation du frontend Nuxt..."
    cd "$FRONTEND_DIR" || { echo "Le dossier frontend n'existe pas"; exit 1; }

    # Corrige les permissions locales
    ...
    sudo tar -czf "$SCRIPT_DIR/$FRONTEND_ARCHIVE" -C .output .
...
EOF
    ...
    # Supprime l'archive locale
    sudo rm "$SCRIPT_DIR/$FRONTEND_ARCHIVE"

    echo "✅ Frontend déployé !"
    cd - >/dev/null
}

# ----------------- FONCTION DEPLOIEMENT BACKEND -----------------
deploy_backend() {
    echo "🔨 Préparation du backend NestJS..."
    cd "$BACKEND_DIR" || { echo "Le dossier backend n'existe pas"; exit 1; }

    # Corrige permissions locales
    sudo chown -R $(whoami):$(whoami) .
    ...
    ssh $VPS_USER@$VPS_HOST << EOF
        ...
        # Supprime tout sauf uploads et .env
...
EOF
    ...
    # Supprime l'archive locale
    sudo rm "$SCRIPT_DIR/$BACKEND_ARCHIVE"
