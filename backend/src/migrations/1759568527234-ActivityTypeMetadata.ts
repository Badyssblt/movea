...
export class ActivityTypeMetadata1759568527234 implements MigrationInterface {
    name = 'ActivityTypeMetadata1759568527234'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`activity\` DROP COLUMN \`duration\``);
        await queryRunner.query(`ALTER TABLE \`activity\` DROP COLUMN \`calories\``);
        await queryRunner.query(`ALTER TABLE \`activity\` DROP COLUMN \`distance\``);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`activity\` ADD CONSTRAINT \`FK_92b8d7ae60a9e36ae170a93b51b\` FOREIGN KEY (\`typeId\`) REFERENCES \`activity_type\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`activity\` ADD CONSTRAINT \`FK_3571467bcbe021f66e2bdce96ea\` FOREIGN KEY (\`userId\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`activity\` ADD \`distance\` decimal(6,2) NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`activity\` ADD \`calories\` int NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`activity\` ADD \`duration\` int NOT NULL`);
    }
